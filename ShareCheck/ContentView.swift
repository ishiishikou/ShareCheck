...
struct DashboardView: View {
    ...
    private var counts: DashboardCounts {
        makeCounts(from: photoService.items)
    }
    ...
    private func makeCounts(from items: [MediaItem]) -> DashboardCounts {
        ...
        for item in items {
            if calendar.isDateInToday(item.creationDate) {
                today += 1
            } else if calendar.isDateInYesterday(item.creationDate) {
                yesterday += 1
            } else if calendar.isDate(item.creationDate, equalTo: now, toGranularity: .weekOfYear) {
                thisWeek += 1
            } else {
                older += 1
            }
        ...
        return DashboardCounts(total: items.count, today: today, yesterday: yesterday, thisWeek: thisWeek, older: older)
    }
}
...
struct PostShareConfirmationView: View {
    ...
    @State private var sharedDecision: Bool = true
    @State private var reviewedDecision: Bool = false
    @State private var skipShared: Bool = false
    @State private var skipReviewed: Bool = false
