...
final class ShareCheckStoreLogicTests: XCTestCase {
    func testMarkSetsSharedAndReviewedStatuses() {
        ...
        let operation = MediaStatusStoreLogic.mark(statuses: &statuses, sharedIds: ["shared-1", "shared-2"], reviewedIds: ["reviewed-1"])
    ...
    func testMarkReturnsNilWhenNoIdsChanged() {
        var statuses: [String: MediaStatus] = [:]
        let operation = MediaStatusStoreLogic.mark(statuses: &statuses, sharedIds: [], reviewedIds: [])
        XCTAssertNil(operation)
        XCTAssertTrue(statuses.isEmpty)
    }

    func testUndoRemovesLatestOperationStatuses() {
        var statuses: [String: MediaStatus] = ["shared-1": .shared, "reviewed-1": .reviewed, "unrelated": .shared]
        let operation = OperationSnapshot(sharedIds: ["shared-1"], reviewedIds: ["reviewed-1"], createdAt: Date())
